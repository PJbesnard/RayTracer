...
class SimpleRayTracer{
    private:
        ...
        */
        ...
        Vector getPixelColor(const Ray& r, const Scene& s, int recursion = 0){
            ...
            Vector P, N;
            bool has_inter = s.intersection(r, P, N, id, t);
            if (has_inter) getDirectLightColor(r, s, recursion, N, P, &pixel_color, id);
            return pixel_color;
        }
         ...
         * \param H : the height
         * \param W : the width
         * \param mode : the mode of the ray tracer (basic render or openGL mode)
         * \param ang : the angle of the camera
         * \param cam : the position of the camera 
         */
        void setPixelColor(const Scene& s, int i, int j, double fov, unsigned char image[1024*1024*3], int H, int W, int mode, double ang[2], double cam[3]){
            //created a random number which follows a Gaussian, it allows to send a ray not necessarily in the center
            double r1 = generator_rand(generator), r2 = generator_rand(generator);
            double dx = sqrt(-2 * log(r1)) * cos(2 * M_PI * r2), dy = sqrt(-2 * log(r1)) * sin(2 * M_PI * r2);
            Vector direction((j - W / 2) + ang[0], (i - H / 2) + ang[1], - W / (2 * tan(fov / 2)));
            ...
            if (mode == 1){
                ...
                image[((H - (H - i - 1) - 1) * W + j) * 3 + 0] = std::min(255., std::max(0., std::pow(color[0], 1 / 2.2))); // red
                image[((H - (H - i - 1) - 1) * W + j) * 3 + 1] = std::min(255., std::max(0., std::pow(color[1], 1 / 2.2))); // green
                image[((H - (H - i - 1) - 1) * W + j) * 3 + 2] = std::min(255., std::max(0., std::pow(color[2], 1 / 2.2))); // blue
            }
    ...
    public:
         ...
         * \param mode : the mode of the ray tracer (basic render or openGL mode)
         */
        void createImage(const Scene& s, const int fov_cam, unsigned char image[1024 * 1024 * 3], int H, int W, double ang[2], double cam[3], int mode){
            double fov = fov_cam * M_PI / 180; // angle de vue
            for (int i = 0; i < H; i++) {
                for (int j = 0; j < W; j++) {
                    setPixelColor(s, i, j, fov, image, H, W, mode, ang, cam);
                }
            }
        }
